...
EXP_HOME_CODE_DIR = '.'
EXP_CONTAINER_CODE_DIR = '/workspace'

# path to Slurm executable
EXP_SLURM_EXECUTABLE = 'benchmarks/scaling_adapters/deployment/slurm.sh'

# path to benchmark executable
EXP_BENCHMARK_EXECUTABLE = 'benchmarks/scaling_adapters/benchmark_serving.py'

# path to container image
EXP_CONTAINER_IMAGE = '/gpfs/scratch/bsc98/bsc098069/llm_benchmarking/images/slora.sif'
        ...
        for benchmark_index, benchmark_args_combination in enumerate(benchmark_args_combinations):
            ...
            if runs is not None:
                for run in range(runs):
                    ...
                    arguments = f"--port='{random_port}' --result-dir='{os.path.join(results_path, run_combination_game)}' {benchmark_args_combination} --launch-server --server-args='--port={random_port} --nccl_port={random_port + 1} {server_args_combination}'"
                    schedule_job(
                        user,
                        queue,
                ...
                arguments = f"--port='{random_port}' --result-dir='{os.path.join(results_path, combination_name)}' {benchmark_args_combination} --launch-server --server-args='--port={random_port} --nccl_port={random_port + 1} {server_args_combination}'"
                schedule_job(
                    user,
                    queue,
                    combination_name,
...
if __name__ == '__main__':
    ...
    with open(config_path, 'w') as config_file:
        config = 'PYTHONPATH=. python ' + ' '.join(sys.argv).replace('{', '"{').replace('}', '}"') + '\n'
